  ...
  methods: {
    ...
    fetchDiscountOptions (data = {}) {
      ...
      if (customer && (customer._id || customer.doc_number)) {
        ...
        const discountByDomain = typeof window === 'object' && window.$discountsDomain &&
        window.$discountsDomain.discount_rules

        if (window._settings && window._settings.domain && Boolean(discountByDomain)) {
          data.domain = window._settings.domain
        }
      }
      modules({
